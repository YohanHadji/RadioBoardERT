...
// WiFi AP SSID
#define WIFI_SSID "rocketteam"
// WiFi password
#define WIFI_PASSWORD "rocket20"
...
void setupInfluxDb();
void plotPointsGse(PacketGSE_downlink packet);
...
void handleLoRaCapsule(uint8_t packetId, uint8_t *dataIn, uint32_t len) {
    ...
    // InfluxDB stream
    #if GSE_DOWNLINK
        PacketGSE_downlink packet;
        memcpy(&packet, dataIn, packetGSE_downlink_size);
        plotPointsGse(packet);
    #endif
#endif
}
...
void handleUartCapsule(uint8_t packetId, uint8_t *dataIn, uint32_t len) {
    ...
    // Point data
        ...
        GSETelemetry.addField("Fill N2O", packet.status.fillingN2O ? 1.0:0.0);
        GSETelemetry.addField("Disconnect Active", (packet.disconnectActive ? 1.0:0.0));
